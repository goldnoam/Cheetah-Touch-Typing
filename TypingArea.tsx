...
const TypingArea: React.FC<TypingAreaProps> = ({ 
  content, userInput, onInputChange, isFinished, 
  theme = 'dark', fontSize = 'md', isPaused = false 
}) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const isDarkMode = theme === 'dark';

  useEffect(() => {
    if (!isFinished && !isPaused) inputRef.current?.focus();
  }, [isFinished, isPaused]);
  ...
  return (
    ...
    >
      <div className={`mono leading-relaxed tracking-wide select-none break-words ${fontSizeClass} ${isPaused ? 'opacity-20 blur-sm' : ''}`}>
        {renderContent()}
      </div>
      ...
      <input
        ...
        value={userInput}
        onChange={(e) => !isFinished && !isPaused && onInputChange(e.target.value)}
        autoFocus
        autoComplete="off"
        spellCheck="false"
        disabled={isPaused}
      ...
      {isPaused && (
        <div className="absolute inset-0 flex items-center justify-center backdrop-blur-[4px] rounded-[2rem] pointer-events-none z-10">
          <p className="text-amber-500 font-black text-3xl tracking-[0.3em] uppercase drop-shadow-lg">
            GAME PAUSED
          </p>
        </div>
      )}

      {!isFinished && !isPaused && userInput.length === 0 && (
